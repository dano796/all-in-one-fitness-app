...
import { FaArrowLeft, FaSearch, FaExternalLinkAlt, FaTimes } from "react-icons/fa";
...
const SearchRecipes: React.FC = () => {
  ...
  const handleSearch = useCallback(
    async (e: React.MouseEvent<HTMLButtonElement> | React.KeyboardEvent<HTMLInputElement>) => {
      e.preventDefault();
      if (!query.trim()) {
  ...
  const infoText = {
    recipeSearch: "Busca recetas por ingrediente, tipo de cocina o plato. Encontrarás información detallada sobre ingredientes, preparación, nutrición y más.",
  };

  const formatNutrition = (nutrition: Recipe["recipe_nutrition"]) => {
    if (!nutrition) return { calories: "N/A", fat: "N/A", carbohydrate: "N/A", protein: "N/A" };
    return {
      calorias: nutrition.calories ? `${nutrition.calories} kcal` : "N/A",
      grasa: nutrition.fat ? `${nutrition.fat} g` : "N/A",
      carbohidratos: nutrition.carbohydrate ? `${nutrition.carbohydrate} g` : "N/A",
  ...
  return (
    <div className="relative min-h-screen bg-[#282c3c] overflow-hidden">
      ...
      <div className="relative z-10 p-4 sm:p-6 space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 max-w-5xl mx-auto px-2 sm:px-4">
          <motion.div
          ...
          <motion.div
            ...
            className="bg-[#3B4252] rounded-lg p-4 sm:p-5 shadow-md w-full"
          >
            <div className="flex items-center mb-3 sm:mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-white mr-2">Buscar Recetas</h3>
        ...
        {hasSearched && !loading && recipes.length === 0 && !error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-gray-300 text-center my-6 sm:my-8 max-w-3xl mx-auto text-sm sm:text-base"
          >
        ...
        {recipes.length > 0 && !selectedRecipe && (
          <motion.div
            ...
            className="bg-[#3B4252] rounded-lg p-4 sm:p-5 shadow-md max-w-5xl mx-auto"
          >
            <h3 className="text-lg sm:text-xl font-semibold text-white mb-3 sm:mb-4">Resultados</h3>
            <div className="space-y-3 sm:space-y-4 max-h-[50vh] overflow-y-auto pr-2 [scrollbar-width:thin] scrollbar-thin scrollbar-track-[#2D3242] scrollbar-thumb-[#6B7280] hover:scrollbar-thumb-[#9CA3AF]">
              {recipes.map((recipe) => (
                <motion.div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => e.key === "Enter" && handleRecipeSelect(recipe)}
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-white text-sm sm:text-base">{recipe.recipe_name}</p>
                    {recipe.recipe_description && (
                      <p className="text-xs sm:text-sm text-gray-300 mt-1 line-clamp-2">{recipe.recipe_description}</p>
                    )}
          ...
          {selectedRecipe && (
            ...
            >
              ...
              >
                <button
                  onClick={handleClose}
                  className="absolute top-3 sm:top-4 right-4 sm:right-6 text-white hover:text-[#ff9404] transition-all duration-300 z-20"
                ...
                {error && (
                  <p className="text-red-400 mb-3 sm:mb-4 text-xs sm:text-sm">{error}</p>
                )}
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {selectedRecipe.recipe_image ? (
                      ...
                      <div className="w-full h-48 sm:h-64 md:h-80 bg-gray-600 rounded-lg flex items-center justify-center">
                        <p className="text-gray-400 text-xs sm:text-sm">Imagen no disponible</p>
                      </div>
                    )}
                  </div>

                  {/* Columna derecha: Detalles */}
                  <div className="overflow-y-auto max-h-[60vh] sm:max-h-[70vh] pr-4 sm:pr-8 [scrollbar-width:thin] scrollbar-thin scrollbar-track-[#2D3242] scrollbar-thumb-[#6B7280] hover:scrollbar-thumb-[#9CA3AF]">
                    {selectedRecipe.recipe_description && (
                      <div className="mb-3 sm:mb-4">
                        <h3 className="text-base sm:text-lg font-semibold text-[#ff9404] mb-1 sm:mb-2">Descripción</h3>
                        <p className="text-gray-300 text-xs sm:text-sm">{selectedRecipe.recipe_description}</p>
                      </div>
                    )}

                    <div className="mb-3 sm:mb-4">
                      <h3 className="text-base sm:text-lg font-semibold text-[#ff9404] mb-1 sm:mb-2">Información</h3>
                      <div className="text-gray-300 text-xs sm:text-sm space-y-1">
                        {selectedRecipe.cooking_time_min && (
                          <p><strong>Tiempo de preparación:</strong> {selectedRecipe.cooking_time_min} minutos</p>
                        )}
                        {selectedRecipe.number_of_servings && (
                          <p><strong>Porciones:</strong> {selectedRecipe.number_of_servings}</p>
                        )}
                        {(selectedRecipe.recipe_types?.recipe_type?.length ?? 0) > 0 && (
                          <p><strong>Tipo:</strong> {selectedRecipe.recipe_types?.recipe_type.join(", ") || "No disponible"}</p>
                        )}
                        {(selectedRecipe.recipe_categories?.recipe_category?.length ?? 0) > 0 && (
                          <p><strong>Categorías:</strong> {selectedRecipe.recipe_categories?.recipe_category.join(", ") || "No disponible"}</p>
                        )}
                      </div>
                    </div>

                    <div className="mb-3 sm:mb-4">
                      <h3 className="text-base sm:text-lg font-semibold text-[#ff9404] mb-1 sm:mb-2">Nutrición (por porción)</h3>
                      <div className="grid grid-cols-2 gap-1 sm:gap-2 text-gray-300 text-xs sm:text-sm">
                        {Object.entries(formatNutrition(selectedRecipe.recipe_nutrition || {})).map(([key, value]) => (
                          <p key={key}>
                    ...
                    <div className="mb-3 sm:mb-4">
                      <h3 className="text-base sm:text-lg font-semibold text-[#ff9404] mb-1 sm:mb-2">Ingredientes</h3>
                      {(selectedRecipe.recipe_ingredients?.ingredient?.length ?? 0) > 0 ? (
                        <ul className="list-disc pl-5 text-gray-300 space-y-1 text-xs sm:text-sm">
                          {selectedRecipe.recipe_ingredients?.ingredient.map((ing: any, index: number) => (
                            <li key={index}>
                              {typeof ing === "string" ? ing : ing.ingredient_name || "Ingrediente desconocido"}
                              {typeof ing !== "string" && ing.quantity && ing.measurement_description
                                ? ` (${ing.quantity} ${ing.measurement_description})`
                                : ""}
                              {typeof ing !== "string" && ing.ingredient_description && ` (${ing.ingredient_description})`}
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-gray-400 text-xs sm:text-sm">Ingredientes no disponibles</p>
                      )}
                    </div>
                  </div>
                </div>
