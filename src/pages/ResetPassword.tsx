...
const ResetPassword = () => {
  ...
  const updatePasswordInDatabase = async (userEmail: string, newPassword: string) => {
    try {
      ...
      if (error) {
        ...
        throw new Error(`Error al actualizar la contraseña en la base de datos: ${error.message}`);
      }
  ...
  const handleResetPassword = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error) {
        console.error("Error al actualizar la contraseña en Supabase Auth:", error.message);
  ...
  return (
    <div className="container mx-auto px-4 py-16 bg-[#282c3c] text-white">
      <div className="max-w-md mx-auto">
        <h1 className="text-4xl font-bold mb-8 text-center">Restablecer Contraseña</h1>
        ...
        {isLoading ? (
          ...
          <div className="bg-[#3B4252] rounded-xl p-8">
            ...
            {!message.includes("Error") && !message.includes("inválido") && !message.includes("verificar") && (
